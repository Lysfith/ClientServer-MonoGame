...
namespace _2016_Project_2.UI
{
    public class UIButton : UIBaseElement
    {
        ...
        public UIButton(Point position, int width, int height, string text, int fontSize = 16)
        {
            Position = position;
            Height = height;
            Width = width;
        ...
        public override void Draw(GameTime gameTime, SpriteBatch spriteBatch)
        {
            if (Enable && Visible)
            {
                ...
                if (HasHover && !ButtonDisabled)
                {
                    ...
                    if (TextCentered)
                    {
                        spriteBatch.Draw(_cursor, new Rectangle((int)(Position.X + (Width - stringSize.X) * 0.5f - 40), (int)(Position.Y + Height * 0.5 - 5), 36, 18), Color.White);
                    }
                    else
                    {
                        spriteBatch.Draw(_cursor, new Rectangle((int)(Position.X + 15 - 40), (int)(Position.Y + Height * 0.5 - 5), 36, 18), Color.White);
                    }
                }
                ...
                if(TextCentered)
                {
                    spriteBatch.DrawString(_font, Text, new Vector2(Position.X + (Width - stringSize.X) * 0.5f, Position.Y + (Height - stringSize.Y) * 0.5f), color);
                }
                else
                {
                    spriteBatch.DrawString(_font, Text, new Vector2(Position.X + 15, Position.Y + (Height - stringSize.Y) * 0.5f), color);
                }
            }
        }
